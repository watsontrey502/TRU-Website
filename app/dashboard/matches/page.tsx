...
interface MatchDisplay {
  ...
  // Extended profile fields for the card
  matched_profile_id?: string;
  matched_age?: number;
  matched_bio?: string;
  matched_interests?: string[];
  matched_drinking?: string;
  matched_smoking?: string;
  matched_intentions?: string;
  matched_neighborhood?: string;
  matched_work?: string;
  matched_avatar_upload_path?: string;
  matched_verification_status?: string;
}

function matchToProfile(match: MatchDisplay): ProfileCardProfile {
  return {
    id: match.matched_profile_id ?? match.event_id,
    first_name: match.matched_first_name,
    age: match.matched_age,
    bio: match.matched_bio,
    interests: match.matched_interests,
    drinking: match.matched_drinking,
    smoking: match.matched_smoking,
    intentions: match.matched_intentions,
    instagram: match.matched_instagram,
    neighborhood: match.matched_neighborhood,
    work: match.matched_work,
    avatar_upload_path: match.matched_avatar_upload_path,
    verification_status: match.matched_verification_status,
  };
}
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-10">
        ...
        <p className="text-white/40 text-sm">
          When both people select each other after an event, it&apos;s a match.
      ...
      {matches.length === 0 ? (
        ...
        >
          ...
          <p className="text-white/40 text-sm max-w-sm mx-auto mb-6">
            Attend events and use Double Take to start matching. When someone
            you picked also picks you, you&apos;ll see them here.
          </p>
          <Link
            href="/dashboard"
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {eventMatches.map((match, i) => (
                  ...
                  >
                    <ProfileCard
                      profile={matchToProfile(match)}
                      variant="compact"
                    />

                    {/* Mutual badge */}
                    <div className="flex items-center justify-center">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-medium bg-gold/10 text-gold border border-gold/20">
                        &#10022; Mutual Match
                      </span>
                    </div>
