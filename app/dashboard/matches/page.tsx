    ...
    <>
      ...
      {matches.length === 0 ? (
        <div className="bg-white rounded-2xl p-10 border border-gray-100 text-center">
          ...
          <h2 className="font-serif text-xl font-semibold text-black mb-2">No matches yet</h2>
        ...
        <div className="space-y-10">
          {Object.entries(grouped).map(([eventName, eventMatches]) => (
            <div key={eventName}>
              <h2 className="font-serif text-lg font-semibold text-black mb-4 flex items-center gap-2">
                  ...
                  <div key={i} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-[var(--shadow-card)]">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-black">{match.matched_first_name}</h3>
                        {match.matched_instagram && (
                          <p className="text-gold text-sm truncate">@{match.matched_instagram.replace("@", "")}</p>
                        )}
                      </div>
                      <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-medium bg-black/10 text-black whitespace-nowrap">
                        It&apos;s mutual!
                      </span>
                    </div>
                  </div>
